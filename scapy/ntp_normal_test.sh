		...
		then
			case "$index" in
				1) # version 1, 2, 3, 4len < 48
					python3 send_scapy.py "IP()/UDP(sport=123,dport=123)/Raw(raw(NTP(version = 1))[:-1])"
			...
			case "$index" in
				1) # ntp len =48
					python3 send_scapy.py "IP()/UDP()/NTP(version = (1,4))"
					python3 send_scapy.py "IP()/UDP()/NTP(version = [2,3])/Raw(b"\x00" * 12)"
					;;
				2) # when 0 < len - 48 < 24,have data no option,len -48 %4 should be 0.
